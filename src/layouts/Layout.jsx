import { Home, FolderOpen, GraduationCap, Menu, X, FileText, Sun, Moon } from 'lucide-react';
...
const Layout = ({ children }) => {
  ...
  const navItems = [
    ...
    { name: 'Experience', path: '#experience', icon: <GraduationCap size={18} /> },
  ];
      ...
      <footer className="border-t border-[var(--border)] py-12 mt-20 bg-[var(--card-bg)] backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="text-center md:text-left">
             <p className="text-[var(--text-h)] font-bold text-xl mb-1">Samet Kenar</p>
             <p className="text-[var(--text)] text-sm italic opacity-80 leading-relaxed max-w-xs">
                Building AI-powered web and desktop applications with passion and precision.
             </p>
          </div>
          <div className="flex space-x-6">
             <a href="https://github.com/sametkenar" target="_blank" rel="noopener noreferrer" className="p-3 border border-[var(--border)] rounded-full text-[var(--text-h)] hover:border-[#6366f1] hover:text-[#6366f1] transition-all">
               <GithubIcon size={22} />
             </a>
             <a href="https://linkedin.com/in/sametkenar" target="_blank" rel="noopener noreferrer" className="p-3 border border-[var(--border)] rounded-full text-[var(--text-h)] hover:border-[#6366f1] hover:text-[#6366f1] transition-all">
